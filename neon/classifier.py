...
from neon.optimizers import Adadelta, Adam
from neon.transforms import Rectlin, Softmax, CrossEntropyMulti, Accuracy
...
parser.add_argument('-vw', '--val_data_dir', default='',
                    help='directory in which to find validation images')
...
val = ClassifierLoader(repo_dir=args.val_data_dir, inner_size=imwidth,
...
'''
'''
#layers.append(Affine(nout=512, init=init, activation=Rectlin(), batch_norm=True))
layers.append(DropoutBinary(keep=0.2))
layers.append(Affine(nout=2, init=init, activation=Softmax()))

...
for idx, filepath in enumerate(real_files):
    ...
    if idx % 1000 == 0:
        print idx
image_paths.append(image_paths_real)
...
"""for idx, filepath in enumerate(paint_files):
    ...
    if idx % 1000 == 0:
        print idx"""
# Sort all images in dir by class
directory='/home/ispick/Projects/illust_detection/detect_natural_images/wikipaintings'
print [x[0] for x in os.walk(directory)][1]
print [x[0] for x in os.walk(directory)][2]
for idx, style_path in enumerate([x[0] for x in os.walk(directory)]):
    if idx == 0:
        continue # style_path='/home/ispick/Projects/illust_detection/detect_natural_images/wikipaintings'
    print style_path

    for file_path in [ f for f in listdir(style_path) if isfile(join(style_path,f)) ]:
        print filepath
        fds
        image_paths[1+idx].append(dir_path + '/' + filepath)
print image_paths[1]
